  ...
  function getCalendar() {
    ...
    return (
      ...
      <div className="mt-4">
        ...
        return (
        <div className="mt-2" style={{cursor:"pointer"}}  key={element.id}>
          <ListGroup>
            <ListGroup.Item variant="primary" onClick={handleShow} className="mb-4 mt-4">{element.real_name}</ListGroup.Item> 
          </ListGroup>
          ...
          <Modal.Body>
            ...
            {element.activity_periods.map(time => {
              console.log(time.start_time)
                  return (
                    <div className="mt-4" key={element.real_name}>
                      <ListGroup>
                        <ListGroup.Item><b>Start time:</b> {time.start_time}</ListGroup.Item> 
                        <ListGroup.Item><b>End time:</b> {time.end_time}</ListGroup.Item> 
                      </ListGroup>
                    </div>
                  )
              })} 
    ...
    <div id="calendar" style={{ height: '500pt',display:"none"}} className="mt-4 mb-4"> 
          <Calendar
            events={resources}
            startAccessor="start"
            endAccessor="end"
